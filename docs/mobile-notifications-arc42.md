...
**Status, September 2026.** The paragraph above describes the state when this section was written and is kept as the record of it; the gap it names is closed. Slice 6 (`25281eb8` on the feature branch, on `main` since #1847 / `91d39abc`) publishes `irrlichtrelay-linux-amd64.tar.gz` and `irrlichtrelay-linux-arm64.tar.gz` with every release from `v0.6.3` on (the relay got its own tarball rather than riding in the daemon's, so `bin/` and `Resources/web/` travel together and the resolver's `../Resources/web` branch finds the dashboard with no `IRRLICHT_UI_DIR`). #1964 added `site/relay.sh`, a one-command installer for that tarball on a Linux host with systemd — architecture match, checksum, layout check, service user, unit, first token, optional Caddy or `tailscale serve` front — driven end to end by `tools/lib/relay-install_test.sh`. `examples/relay/DEPLOY.md` and `site/docs/elfdans.html` lead with it. Still open: no darwin relay build, so Shape B remains a from-source install, and the container image stays deferred.
