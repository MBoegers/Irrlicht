...
# Overridable so a mutated copy of the installer can be graded by the same
# cases (the PR that added this file records which mutations went red).
...
#   flat    the binary at the tarball root, no bin/, no Resources/ — the
#           layout a `tar` of a lone binary produces, and the one that makes
#           the relay answer 503 on /
make_release() {
    local out="$1" mode="$2"
    local staging="$out/staging"
    rm -rf "$staging"; mkdir -p "$out"
    if [[ "$mode" == flat ]]; then
        mkdir -p "$staging"
        make_relay_stub "$staging/irrlichtrelay"
    else
...
# new_env <name> — a fake machine: stubs on PATH, a fixture release, and the
# four redirected install locations. Echoes the root.
new_env() {
    local root="$WORK/$1"
...
# ===========================================================================
# 1. THE HAPPY PATH, loopback. Everything the runbook did by hand lands where
#    the runbook put it, and the printout is the Mac's two fields plus the
#    toggle, in the form the Settings pane expects.
# ===========================================================================
root="$(new_env happy)"
...
assert_contains "$sysd" "daemon-reload" "1 happy: systemctl daemon-reload"
assert_contains "$sysd" "enable --now irrlichtrelay" "1 happy: unit enabled and started"
...
# The protocol pin has to travel with every download, or a hijacked redirect
# can hand us a plaintext substitute (site/install.sh's fetch comment).
...
# ===========================================================================
# 2. RE-RUN (upgrade). Stops the running relay, replaces the prefix, keeps the
#    tokens — a fresh token per upgrade leaves the operator with a growing
#    list and no idea which one the Mac holds.
# ===========================================================================
...
assert_eq "$(logof "$root" useradd | wc -l | tr -d ' ')" "1" "2 rerun: useradd ran once across both runs"
...
out="$(run_installer "$root" --)"; rc=$?
[[ $rc -ne 0 ]] && pass "3 badsum: exits non-zero" || fail "3 badsum: exits non-zero" "rc=0"
...
assert_eq "$(logof "$root" useradd | wc -l | tr -d ' ')" "0" "3 badsum: no user created"

# ===========================================================================
# 4. FLAT TARBALL (a lone binary, no bin/, no Resources/web/) must be caught
#    here rather than produce a relay that answers 503 on /.
# ===========================================================================
root="$(new_env flat flat)"
out="$(run_installer "$root" --)"; rc=$?
[[ $rc -ne 0 ]] && pass "4 flat: exits non-zero" || fail "4 flat: exits non-zero" "rc=0"
assert_contains "$out" "tarball layout" "4 flat: names the layout as the problem"
assert_file_absent "$root/opt/irrlichtrelay" "4 flat: nothing installed"
assert_not_contains "$(logof "$root" systemctl)" "enable" "4 flat: nothing enabled"
...
[[ $rc -ne 0 ]] && pass "5 arch: exits non-zero" || fail "5 arch: exits non-zero" "rc=0"
assert_contains "$out" "riscv64" "5 arch: names the architecture it saw"
assert_eq "$(logof "$root" curl | wc -l | tr -d ' ')" "0" "5 arch: refused before any download"
...
[[ $rc -ne 0 ]] && pass "6 darwin: exits non-zero" || fail "6 darwin: exits non-zero" "rc=0"
assert_contains "$out" "Shape B" "6 darwin: points at the Mac shape instead"
assert_eq "$(logof "$root" curl | wc -l | tr -d ' ')" "0" "6 darwin: no download"

root="$(new_env nosystemd)"
out="$(run_installer "$root" STUB_SYSTEMD_STATE=offline --)"; rc=$?
[[ $rc -ne 0 ]] && pass "6 nosystemd: exits non-zero" || fail "6 nosystemd: exits non-zero" "rc=0"
...
[[ $rc -ne 0 ]] && pass "6 notroot: exits non-zero" || fail "6 notroot: exits non-zero" "rc=0"
assert_contains "$out" "sudo" "6 notroot: tells the user to re-run with sudo"
assert_eq "$(logof "$root" curl | wc -l | tr -d ' ')" "0" "6 notroot: no download"
...
assert_eq "$(logof "$root" firewall-cmd | wc -l | tr -d ' ')" "0" "7 domain: NEVER ran firewall-cmd"
...
# Same run again: the Caddyfile import must not be appended twice.
out="$(run_installer "$root" -- --domain relay.example.com)"; rc=$?
assert_eq "$rc" "0" "7 domain rerun: exits 0"
assert_eq "$(grep -c 'import ' "$root/etc/caddy/Caddyfile")" "1" "7 domain rerun: one import line, not two"
...
# --domain with no Caddy and no known package manager refuses by name rather
# than guessing at a package manager.
root="$(new_env domain-nocaddy)"
out="$(run_installer "$root" -- --domain relay.example.com)"; rc=$?
[[ $rc -ne 0 ]] && pass "7 domain nocaddy: exits non-zero" || fail "7 domain nocaddy: exits non-zero" "rc=0"
...
out="$(run_installer "$root" -- --tailscale)"; rc=$?
[[ $rc -ne 0 ]] && pass "8 tailscale missing: exits non-zero" || fail "8 tailscale missing: exits non-zero" "rc=0"
...
[[ $rc -ne 0 ]] && pass "8 both: --domain and --tailscale together refuse" || fail "8 both: --domain and --tailscale together refuse" "rc=0"

# ===========================================================================
# 9. --uninstall reverses what an install wrote and keeps the state dir;
...
assert_not_contains "$(cat "$root/etc/caddy/Caddyfile")" "irrlichtrelay" "9 uninstall: Caddyfile import and marker stripped"
...
assert_eq "$(logof "$root" userdel | wc -l | tr -d ' ')" "0" "9 uninstall: user kept with the state"
...
[[ $rc -ne 0 ]] && pass "9 purge alone: refuses without --uninstall" || fail "9 purge alone: refuses without --uninstall" "rc=0"

# ===========================================================================
# 10. The installer must stay POSIX-sh clean — it reaches users as
...
if [[ -n "$POSIX_SH" ]]; then
    if "$POSIX_SH" -n "$RELAY_SH" 2>/dev/null; then
        pass "10 syntax: site/relay.sh parses under $POSIX_SH (real POSIX shell; the cases above also ran under it)"
    else
        fail "10 syntax: site/relay.sh parses under $POSIX_SH"
    fi
else
    if sh -n "$RELAY_SH" 2>/dev/null; then
        pass "10 syntax: site/relay.sh parses under /bin/sh (NO dash here — syntax only, and the cases above ran under bash-as-sh)"
    else
        fail "10 syntax: site/relay.sh parses under /bin/sh"
    fi
fi
assert_eq "$(head -n 1 "$RELAY_SH")" "#!/bin/sh" "10 shebang: #!/bin/sh, so tools/posix-lint.sh scopes it in"
